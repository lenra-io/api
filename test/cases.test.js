...
const casesPath = resolve('test', 'cases');
const tests = readdirSync(casesPath).filter(f => f.endsWith('.yml'));

describe("Unify", () => {
    tests.forEach(testCase => {
        it(`Validate ${testCase}`, async () => {
            const test = YAML.parse(readFileSync(resolve(casesPath, testCase), 'utf-8'));
            const schemaPromise = getSchema(resolve("src", test.schema));
            const content = JSON.parse(readFileSync(resolve(casesPath, testCase.replace(/yml$/, "content.json")), 'utf-8'));
